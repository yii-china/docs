# 将 Yii 应用程序部署到 Docker Swarm 和 Traefik

本指南将引导您从空白服务器开始，使用 [Traefik](https://traefik.io/traefik) 作为反向代理，并从容器注册表（[Forgejo](https://forgejo.org/) 或 [Gitea](https://about.gitea.com/)）部署，将 Yii 应用程序部署到 [Docker Swarm](https://docs.docker.com/engine/swarm/)。
...
将 `<YOUR_SERVER_IP>` 替换为您服务器的公共 IP 地址。

### 设置反向代理网络

创建一个专用的覆盖网络，供反向代理与您的服务通信：
...
使用 Traefik 时，您需要修改 Yii 应用程序的 `docker/prod/compose.yml` 以使用 Traefik 标签而不是 Caddy 标签：
...
不要使用环境变量，而是对敏感信息使用 Docker secrets：
...
- 检查日志：`docker -H ssh://docker-web service logs traefik_traefik`
...
您已成功将 Yii 应用程序部署到 Docker Swarm，具有：
- 容器注册表（Forgejo 或 Gitea）
- 通过 Traefik 自动启用 HTTPS
- 通过滚动更新实现零停机部署
- 通过多个副本实现高可用性
