...
在构建现代应用程序时，您经常需要向外部 API 发起 HTTP 请求。本文演示如何在 Yii3 应用程序中使用 Guzzle 和 [PSR 接口](https://www.php-fig.org/psr/)发起 HTTP 请求。
...
使用这些接口可确保您的代码与框架无关，并遵循已建立的 PHP 标准。
...
return [
    ...
    // 配置 PSR-17 工厂 - 这些将取决于您选择的 PSR-7 实现
...
Guzzle 支持用于横切关注点（如身份验证、日志记录或重试）的中间件：
...
为了在发起多个请求时获得更好的性能，您可以使用异步请求：

> **注意**：异步功能不是 PSR 接口的一部分，因此此代码明确依赖于 Guzzle。
...
在测试发起 HTTP 请求的服务时，您可以使用 Guzzle 的 MockHandler：
...
6. **在测试中使用模拟**：使用 Guzzle 的 MockHandler 或类似工具来测试您的 HTTP 客户端代码，而无需发起真实的网络请求。

通过遵循这些模式并使用 PSR 接口，您将在 Yii3 应用程序中创建可维护、可测试和可互操作的 HTTP 客户端代码。
