...
对于 Yii3，包的数量显著增加，以实现更多的可重用性和独立发布。
为了简化框架本身的开发，我们创建了一个特殊的工具，可从 [yiisoft/yii-dev-tool](https://github.com/yiisoft/yii-dev-tool) 获取。
...
1. 安装/更新 [`packages.php`](https://github.com/yiisoft/yii-dev-tool/blob/master/packages.php) 中列出的所有包，
   或者如果指定了该列表中的单个包。
2. 对于每个已安装的包，检查 `vendor` 目录中是否有 `packages.php` 中列出的包。
   如果有，将包目录替换为指向另一个包源的符号链接。

因此，你将拥有许多相互使用的包，因此在开发过程中无需 `git push` 和 `composer install` / `composer update`。

工具的 [详细示例](https://github.com/yiisoft/yii-dev-tool#usage-example) 可在其 README 中找到。
