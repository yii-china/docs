...
您可以使用别名来表示文件路径或 URL,这样您就不必在项目中硬编码绝对路径或 URL。别名必须以 `@` 字符开头,以便与普通文件路径和 URL 区分开来。定义时没有前导 `@` 的别名将自动添加 `@` 字符前缀。

默认的 Yii 应用程序在 `config/params.php` 中预定义了一些别名。例如,别名 `@public` 表示 Web 根路径;`@baseUrl` 表示当前运行的 Web 应用程序的基础 URL。
...
给定一个已定义的别名,您可以通过附加斜杠 `/` 后跟一个或多个路径段来派生新别名。
例如,`@foo` 是根别名,而 `@foo/bar/file.php` 是派生别名。
...
`yiisoft/aliases` 参数从 [`yiisoft/aliases` 包](https://github.com/yiisoft/aliases)初始化 `Aliases` 服务。
您可以在运行时使用该服务设置额外的别名:
...
最好在配置级别解析别名,这样服务就可以获得准备好使用的 URL 和路径字符串:
...
您可以使用 `Aliases` 服务将别名或派生别名解析为它所表示的文件路径或 URL:
...
派生别名所表示的路径/URL 是通过将派生别名中的根别名部分替换为其对应的路径/URL 来确定的。

> [!NOTE]
> `get()` 方法不检查结果路径/URL 是否指向现有文件或资源。
...
如果 `@foo/bar` 没有被定义为根别名,最后一条语句将显示 `/path/to/foo/bar/file.php`。
